...
import { Clock, CheckCircle2, Truck, PackageCheck, AlertCircle, RefreshCw, ClipboardList, MapPin, Phone, User, ShoppingCart, HelpCircle } from 'lucide-react';
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between">
                      ...
                      {/* Status badge */}
                      <span className={`px-2.5 py-0.5 text-[9px] font-bold uppercase tracking-widest ${
                    ...
                    <div className="mt-4 flex items-center justify-between text-xs border-t border-[#EEEEEE] pt-3">
                      <div>
                        ...
                        <span className="block font-bold text-black font-mono mt-0.5">${order.totalAmount.toFixed(2)}</span>
                      </div>
          ...
          {activeOrder ? (
            <div className="space-y-6" id="active-tracking-details">
              ...
              <div className="rounded-none border border-[#EEEEEE] bg-white p-6 sm:p-8">
                ...
                {activeOrder.status === 'cancelled' ? (
                  <div className="my-6 flex items-start space-x-3 rounded-none border border-black bg-[#FDFDFD] p-5 text-black">
                    ...
                    <div>
                      ...
                      <p className="text-xs mt-2 text-[#717171] leading-relaxed">
                        This purchase request has been flagged as cancelled and no further action will occur. Please create a new cart order if desired.
                      </p>
                    </div>
                  </div>
                ) : (
                  ...
                  <div className="my-10 relative">
                    {/* Line Connector */}
                    <div className="absolute top-5 left-6 right-6 h-0.5 bg-[#EEEEEE] -z-0 hidden md:block" />Overlay
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6 md:gap-4 relative z-10">
                      {steps.map((step, index) => {
                        ...
                        return (
                          <div key={step.status} className="flex md:flex-col items-center md:text-center space-x-4 md:space-x-0">
                            {/* Circle Node */}
                            <div className={`flex h-11 w-11 items-center justify-center rounded-full border transition-colors ${
                              isCompleted
                                ? 'bg-black border-black text-white'
                                : 'bg-white border-[#EEEEEE] text-[#717171]'
              ...
              <div className="rounded-none border border-[#EEEEEE] bg-[#FDFDFD] p-6 sm:p-8 space-y-4">
                <div className="flex items-start space-x-3 text-black">
                  ...
                  <div>
                    <h4 className="text-xs font-bold uppercase tracking-[0.15em] text-black">Seller & Courier Simulator Sandbox</h4>
                    <p className="text-xs mt-2 text-[#717171] leading-relaxed">
                      Standard users do not update shipping status. However, to help you test the live tracking progress, use these controls to manually advance this order's state!
                    </p>
                  </div>
                </div>
                ...
                <div className="rounded-none border border-[#EEEEEE] bg-white p-6">
                  ...
                  <div className="mt-4 space-y-3 text-xs leading-relaxed">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="font-bold text-black uppercase tracking-wide text-xs">{activeOrder.userName}</span>
                    </div>
                    <div className="flex items-center space-x-2">
                ...
                <div className="rounded-none border border-[#EEEEEE] bg-white p-6">
                  ...
                  <div className="mt-4 divide-y divide-[#EEEEEE] max-h-[140px] overflow-y-auto pr-1">
                    {activeOrder.items.map((item, idx) => (
                      <div key={item.productId + '-' + (item.selectedSize || idx) + '-' + (item.selectedVariant?.id || idx)} className="flex flex-col py-2 border-b border-[#F9F9F9] last:border-0">
                        <div className="flex justify-between text-xs">
                          ...
                          <span className="font-bold text-black font-mono">${(item.price * item.quantity).toFixed(2)}</span>
                        </div>
                  ...
                  <div className="mt-3 border-t border-[#EEEEEE] pt-3 flex items-baseline justify-between">
                    <span className="text-[10px] uppercase tracking-wider text-[#717171] font-bold">Payment Status</span>
                  ...
                  <div className="mt-1 flex items-baseline justify-between">
                    ...
                    <span className="text-base font-bold text-black font-mono">${activeOrder.totalAmount.toFixed(2)}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-none border border-dashed border-[#EEEEEE] p-12 text-center bg-white flex flex-col items-center justify-center min-h-[350px]">
              ...
              <p className="mt-2 text-xs text-[#717171] max-w-sm leading-relaxed">
                Click on an order from your history list on the left to display its active shipping timeline and access testing controls.
              </p>
            </div>
          )}
        </div>
